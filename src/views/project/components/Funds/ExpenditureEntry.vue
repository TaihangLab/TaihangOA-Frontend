<template>
  <el-dialog :model-value="visible" @update:model-value="updateVisible" title="支出录入" width="90%" @close="closeExpenselEditDialog">
    <div style="max-height: 700px;">
      <el-row :gutter="10" class="mb8">
        ...
        <el-col :span="1.5">
          <el-button type="info" plain icon="upload" size="mini" @click="handleImport"
                     v-hasPermi="['system:user:import']">导入</el-button>
        </el-col>
        <el-col :span="20">
          <div style="display: flex; justify-content: flex-end; margin-right: 5px;" >
            <el-button type="primary" size="mini" @click="addFunds">
              提交
      ...
      <!--新增支出录入-->
      <ExpenditureAdd :visible="isExpenditureAddDialogVisible"  @new-data="handleNewData" @close:visible="isExpenditureAddDialogVisible = $event" @update:visible="isExpenditureAddDialogVisible = $event"></ExpenditureAdd>
      <!--导入支出录入表-->
...
const firstLevelSubjectFormatter = (row: { firstLevelSubject: number }) => {
  const firstLevelSubject = [
    '设备费', '业务费', '劳务费', '材料费', '科研活动费', '科研服务费', '人员和劳务补助费',
    '绩效支出', '管理费', '房屋租赁费', '日常水电暖费', '资料费', '数据样本采集费',
    '测试化验加工费', '燃料动力费', '办公费', '印刷/出版费', '知识产权事务费', '车辆使用费',
    '差旅费', '会议/会务费', '专家咨询费', '其他费用'
  ];
...
const secondLevelSubjectFormatter = (row: { secondLevelSubject: number }) => {
  const secondLevelSubject = [
    '购置设备费', '试制设备费', '设备升级改造费', '设备租赁费', '材料费', '资料费', '数据样本采集费',
    '测试化验加工费', '燃料动力费', '办公费', '印刷/出版费', '知识产权事务费', '车辆使用费',
    '出版/文献/信息传播/知识产权事务费', '差旅费', '会议/会务费', '国内协作费', '国际合作交流费',
    '专家咨询费', '人员劳务费', '会议/差旅/国际合作与交流费', '无'
  ];
...
const thirdLevelSubjectFormatter = (row: { thirdLevelSubject: number }) => {
  const thirdLevelSubject = [
    '无', '知识产权事务费', '印刷打印制作费', '文献数据库费', '信息传播费', '会议费', '差旅费', '国际合作费'
  ];
...
// 查看预算及支出汇总
const checkOther = () => {
  fundsAndBalance(props.projectId)
    .then((resp) => {
      // 处理获取到的数据
    });
};
